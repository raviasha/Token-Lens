...
type Scope = { sessions: { platform: string; sessionId: string }[] };
...
export function registerTaskCards(context: vscode.ExtensionContext, output: vscode.OutputChannel): void {
  ...
  function html(card: Card | null, error = ''): string {
    ...
    const scope = card?.scope.sessions.map(s => `${s.platform}: ${s.sessionId}`).join(', ') || 'Choose a session';
  ...
  async function chooseScope(): Promise<void> {
    const sessions = await cli('sessions') as { platform: string; sessionId: string; timestamp: string }[];
    const picked = await vscode.window.showQuickPick(sessions.map(s => ({ label: `${s.platform} · ${s.sessionId}`, description: s.timestamp, value: s })), { placeHolder: 'Select the conversation for this card' });
    if (!picked) return;
    const card = await cli('create', JSON.stringify({ sessions: [{ platform: picked.value.platform, sessionId: picked.value.sessionId }] }));
    cardId = card.id;
    await refresh();
  }
