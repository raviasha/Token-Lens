...
const { createCard, saveRevision, loadCard, correctClaim, listCards, prepareGeneration, generationScope, loadOrCreateLiveCard, loadOrCreateWorkspaceCard } = require('./card.cjs');
...
function main(argv) {
  ...
  if (command === 'workspace') return loadOrCreateWorkspaceCard(workspace);
